...
public non-sealed class GitOperationServiceImpl implements GitOperationService {
    ...
    @Override
    public boolean repositoryExists(String namespace) {
        try {
            validationService.validateNamespace(namespace);
            ...
            boolean exists = gitDir.exists() && gitDir.isDirectory();
            log.debug("Repository exists check for namespace '{}': {}", namespace, exists);
            return exists;
        } catch (Exception e) {
            log.debug("Repository exists check failed for namespace '{}': {}", namespace, e.getMessage());
            return false;
        }
    }

    @Override
    public void validateRepositoryHealth(String namespace) {
        validationService.validateNamespace(namespace);
        ...
        if (!gitDir.exists() || !gitDir.isDirectory()) {
            log.error("Git repository not properly initialized for namespace: {}", namespace);
            throw GitOperationException.repositoryAccessFailed(namespace,
                    new IOException("Git repository not found or corrupted"));
        }

        // Try to open the repository to ensure it's not corrupted
        try (Git git = Git.open(namespaceDir)) {
            // If we can open it, the repository is healthy
            log.debug("Repository health check passed for namespace: {}", namespace);
        } catch (IOException e) {
            log.error("Repository health check failed for namespace '{}': {}", namespace, e.getMessage());
            throw GitOperationException.repositoryAccessFailed(namespace, e);
        }
    }
}